...
function UsersList({ usersList, searchText }) {
  ...
  const searchedUsers = usersList && usersSortedAlphabetically.filter(user => user.first_name.toLowerCase().includes(searchText) || user.last_name.toLowerCase().includes(searchText))
  ...
  return (
    <>
    {!searchText && usersList && usersList.map(user => {
    ...
    {searchText && usersList && searchedUsers.map(user => {
      const {first_name, last_name, avatar, id} = user
      return (
        <User {...{ key:id, firstName:first_name, lastName:last_name, avatar, id, checked, setChecked, itemsSelected, setItemsSelected }} />
      )
